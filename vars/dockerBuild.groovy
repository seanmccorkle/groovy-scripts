def call() {
    ...
    def directories = sh(script: 'cd ${APP_ORIGIN_WORKSPACE}/app && find * -maxdepth 0 -type d || true', returnStdout: true).trim().split('\n')
    ...
    def dockerfiles = sh(script: 'cd ${APP_ORIGIN_WORKSPACE}/app && find */Dockerfile || true', returnStdout: true).trim().split('\n')
    node {
